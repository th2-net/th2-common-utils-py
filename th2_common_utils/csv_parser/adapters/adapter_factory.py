...
def create_adapter(filename: str, csv_version: str) -> AbstractCsvStreamAdapter:
    if filename.endswith("matrix.csv") or filename.endswith("Matrix.csv"):
        return PredictionCsvStreamAdapter(csv_version)
    if filename.endswith("input.csv") or filename.endswith("Input.csv"):
        return InputCsvStreamAdapter(csv_version)
